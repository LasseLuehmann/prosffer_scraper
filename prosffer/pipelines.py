...
class SupermarketScraperPipeline:
    ...
    def refine_category_with_context(self, product_name, original_category):
        ...
        """
        # Extended keyword priority rules with German terms
        keyword_priority = [
    ...
    def map_category(self, store, category, product_name):
        ...
        if isinstance(category, list):
            ...
            # Refine category based on product name if no match found
            return self.refine_category_with_context(product_name, 'Other')
        else:
            # If it's a single category, map it and refine with context-based logic
...
class PostgresPipeline:
    ...
    def process_item(self, item, spider):
        try:
            ...
            self.conn.commit()  # Commit the transaction after successful execution
